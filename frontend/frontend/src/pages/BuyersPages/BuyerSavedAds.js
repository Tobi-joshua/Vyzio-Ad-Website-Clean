...
export default function BuyerSavedAdsList() {
  ...
  // Fetch saved ads on mount or userId/token change
  useEffect(() => {
    if (!token || !userId) return;

    setLoading(true);
    fetch(`${API_BASE_URL}/api/buyer/${userId}/saved-ads/`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
      .then((res) => {
        if (!res.ok) throw new Error("Failed to load saved ads");
        return res.json();
      })
      .then((data) => {
        setSavedAds(data);
      ...
      .catch((err) => {
        setError(err.message || "Error loading saved ads");
        setLoading(false);
      });
  }, [token, userId]);
