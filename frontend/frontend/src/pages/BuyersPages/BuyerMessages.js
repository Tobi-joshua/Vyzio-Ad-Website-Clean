...
export default function BuyerMessages() {
  const { userId, userAvatar, firstName } = useContext(BuyerDashboardContext);
  ...
  const fetchChats = () => {
    fetch(`${API_BASE_URL}/api/buyer/${userId}/chats/`)
      .then((res) => {
    ...
    <Box sx={{ maxWidth: 600, mx: "auto", mt: 4 }}>
      ...
      >
        ...
        <Stack direction="row" spacing={1}>
          <Button variant="contained">Browse Ads</Button>
        </Stack>
      </Box>
...
>
  <ListItemAvatar>
    ...
    >
      <Avatar>{chat.seller_name.charAt(0)}</Avatar>
    </Badge>
  </ListItemAvatar>
  <ListItemText
    ...
    primary={`${chat.seller_name} — ${chat.ad_title}`}
